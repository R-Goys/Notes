...
6. 调度循环中，每次调度都会从全局和本地的运行队列中查找goroutine，如果都没找到，就会进行阻塞性的查找：从本地，全局运行队列中找（这里会从全局队列批量窃取本地队列长度一半数量的 goroutine 提高窃调度效率，同时尽量负载均衡），如果还是没有，那么就会非阻塞监听一下 netpoll，如果有事件就绪就万事大吉，没有的话，就会开始工作窃取，如果其他 p 队列上依旧不存在可以调度的 goroutine，那么此时会看看垃圾回收是否有空闲的 worker 没有被调度，否则会去一定程度阻塞地监听 netpoll，如果依旧没有，就会再跑一遍 `findRunnable`。
...
4. **基于信号的抢占式调度**：通过 `SIGURG` 信号和修改寄存器，强制让出 CPU，解决紧密循环 Goroutine 无法被调度的问题，提高调度公平性和 GC 效率，常见的抢占点有，函数调用时出发 morestack 会检测是否有信号。
...
表面上是切片，内部使用四叉堆维护timer(计时器)，堆顶元素是距离截止时间最近的计时器，四叉堆，顾名思义，就是每个节点只能有四个子节点，目前 1.26 是在每次执行调度的时候会协助检查全局的四叉堆的 timer 是否需要触发来实现的，当需要触发某一个 timer 的时候，就会通过往计时器结构体中的管道里面发送一个数据来实现定时器的功能，内部通过 select 实现无信号积压的情况。
...
Go 的网络轮询器其实就是为网络 I/O 做了一层优化，深入到 `net/http` 的源码我们可以发现，他就是基于 pollDesc 进行实现的监听操作，而 go 的网络轮询器在 linux 上的实现就是利用 epoll 的多路复用来监听网络 I/O，然而，其实这和我们自己实现一个 reactor 模式的网络库并没有本质区别，go 的网络轮询器就是在一些固定的点去制定 netpoll，去帮你监听而已，`net/http` 和 `hertz` 的底层并不一样，`hertz` 的高性能并没有去依赖 Go 中内置的网络轮询器，而是自己实现了一个名叫 `netpoll` 的网络库，其实就是一个多 reactor 模式 + 协程池 + buffer 复用。
...
- pollWork，这个函数会在 gcDrain 中被调用，原因是 gcDrain 表示的是从 gcWork 里面拉取任务来进行三色标记任务，返回后会把未执行完的 work 归还，其执行的优先级很低，所以当遇到其他任务，如网络事件，计时事件触发就需要及时返回去执行他们，而不是在这里执行垃圾标记。
...
在扩容期间发生读写操作怎么办？此时我们的hash结构体中存在 `oldbuckets` 字段，在还没有分流完的时候存储旧桶的数据，扩容期间发生读写，会优先从这里读写数据；并且实际上，我们的哈希表扩容是渐进式迁移，每次发生写或者删除操作，都会进行渐进式迁移，防止一次性扩容 + 迁移导致阻塞。
...
bmap 定义只包含 key 的哈希值前八位的数字，以加速查询，为什么不直接比较 key 呢？因为一个 key 可能是一个大数字，也可能是很长的字符串，cpu 直接比较耗时相对于只比较一个字节，损耗明显低了不小；除此之外，考虑到局部性原则，key 哈希值前八位这一串是通过数组遍历进行比较的，位于同一个 cache line，cache 命中率高，所以性能也会更高，而直接比较 key 性能就会低很多。所谓拉链法，并不是说将每个 kv 都作为一个链表的节点，其实就是将 bmap bucket 当作的节点才对，每个节点包括 8 个 KV。


这里得纠正一下，cacheline 通常为 64 字节而不是 64 bit，这里纠正一下，但是值得一提的是，新版的 swiss map 用的 tophash 比对还加上了 simd 加速贼几把帅
...
很多字段我们并不能仅凭名字就可以推断出它的作用，但是部分字段还是和原始的哈希表是一致的，比如 seed/KeyHash ，但都不是我们今天的重点，想要知道这些字段有什么作用，我们可以进一步去看看 sync.Map 的方法，比如 Store ， Store 的作用是将键值对存储在这个哈希表中，除去一些封装，他在最后会调用一个 swap 的方法，这也是最核心的部分。
...
```
为什么哈希 bits 都没有使用完，直接 break 了？难道直接就用前几位就直接插入了吗？因为我最开始有这样的疑惑，所以这里还涉及到一个关键的函数，可以注意到，当发生哈希冲突的时候（意味着 lazy expand），我们在最后进行 Store 的时候，有一个函数叫做 expand，实际上，expand 会利用我们传入的 hash 和 hashshift 继续构造我们的子树，当然，如果哈希值完全冲突，那么会直接挂在溢出桶上，所以并不会有上述所说的情况：
```go
...
```
可以看到，我们的 sync.Map 的实现中是以哈希前缀树组织的，其中每个节点的数据结构如下：
...
于是，1.23 版本之前，在读多写少场景下，我们可以使用具有大量写优化的 sync.Map，高并发写的时候还是使用读写锁 + map 更好。至于 1.23 之后，我觉得性能已经得到了很大的优化，锁的粒度也更小了，所以我觉得无论什么场景都能应对。
...
```
这意味着什么，`interface{}` 并不是 any，也就是说并不是任何类型，而是一个实际的类型，当我们将其作为参数传递给函数的时候，我们实际的参数的类型会发生隐式转换，变成所谓的 `interface{}` 类型，原本的 nil 到了函数中并不是 nil 了，我们有时会遇到需要将普通的 `interface{}` 类型进行类型断言，这里是通过运行时内部的一个 `getitab` 方法实现的，通过传入接口和断言的结构体类型，然后用全局的 `itabTable` 的 `find` 方法里面确定唯一的 `itab` 的，由于只通过哈希值 `_type` 里面的肯定是不行的，它这里通过二次线性探测法解决哈希冲突的问题。这样下来就得到一个 `itab` 进而可以进行方法调用，值得一提的是，所有的类型断言都会加入到这个全局的 `itabTable` 缓存里面，即便他是失败的，我的理解是它充当了一个缓存的作用。
```go
...
```
`tab` 存储着这个类型的可以调用的方法。我们需要调用接口的方法的时候，会通过 `data` 直接调用 `tab` 里面的 `fun` 成员去调用对应的方法，如果 `itab` 里面的 `fun[0] == 0` 则代表这个类型没有实现这个接口。

在类型断言的时候，iface 还会通过 `tab` 中的 `hash` 成员来判断类型是否一致，以此来实现类型转换，实际上这里的类型转换是在全局的 `itabTable` 的 `find` 方法里面确定唯一的 `itab` 的，由于只通过哈希值肯定是不行的，它这里通过二次线性探测法解决哈希冲突的问题。

**动态派发机制**是运行期间选择具体多态操作执行的过程，也就是接口调用方法的时候，如果不能确定这个对象具体的类型，就会在运行时通过 `tab` 中的成员 `fun` 决定调用这个方法的哪个实现，这虽然会带来一定开销，但是并不算太大，然而，如果我们方法的接收对象为结构体而不是结构体指针，那么在动态派发的过程中还涉及到数据拷贝，于是开销就会很大。

## 23. go: linkname 是什么？

有的 runtime 函数前会有一个
```go
//go:linkname func1 pkg1/func2
func func1() {
	// 有函数体
}
```
这就是所谓的 linkname，他的意思是要告诉链接器，当你在 pkg1 找到 func2 的时候，请不要管他的函数体，而是将他的函数体重定向到 func1，这个称之为推模式。

当然，前面有 linkname 的函数也可以是空函数体实现，此时的意思就是说，调用这个函数的时候，请到 pkg1/func2 找到这个函数的实现来进行调用，这个就叫拉模式。

一般建议是两个一起用，语义更加明确，在某些场景下，我们可能需要极致的性能，有很多第三方库比如字节的 sonic 就使用了 linkname 技术调用了 runtime 的内部代码进行优化，但是也被 go 语言官方挂了）

缺点就是它破坏了包内部细节的封装，如果我们用它链接了 go 内部的包，不同版本之间可能会产生不兼容，因此使用这种黑科技需要慎重。

## 24. 反射

反射常见于 RPC 框架，Json 编解码等地方，当然如果有自己实现了胶水代码，则可以不用依赖反射。反射通常用于在运行时去获取某个不确定对象的数值、类型、函数调用等信息，它本质上是通过 `interface{}` 进而去访问某个值的 `_type` 和 `data`，我们可以将 `Value` 的数据看作为 `eface` 当我们需要获取并调用其方法的时候，会尝试将他转换为 `iface` 并去获取他的 `itab` 成员，进而获取到某个特定方法的指针，从而实现调用。

不建议在业务开发中使用反射的主要原因在于性能，他会涉及到大量的类型转换以及拷贝逻辑，所以才会引发一定的性能问题，并且我们如果通过反射去访问某一个值，他也会逃逸到堆上，进而引发 GC 压力。

个人见解，业务开发常见瓶颈不在于反射而是网络 IO，反射通常影响不大，反射相关的性能瓶颈及其优化常见于计算密集型或者 infra 框架。