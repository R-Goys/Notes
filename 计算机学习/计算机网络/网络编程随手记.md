...
对于简单的一次连接建立这样就行了，然而，现实中我们的服务器可能会在同一时间面对大量的连接，此时就需要我们的 I/O 模型来监听这一事件，常见的 I/O 模型有我们熟悉的 epoll，我们通过 epoll 这组系统调用接口同时监听多个客户端的响应，一旦有新的请求到达，就会将这些请求交给工作线程/协程去处理刚刚到达的请求。值得注意的是，我们常说的 TCP 三次握手就是在 connect 的时候实现的。
...
Reactor 要求主线程只负责监听文件描述符上是否有事件发生，有的话就会将该事件通知工作线程，读写数据，接受新的连接均在工作线程中进行，比如 go 的网络轮询器，字节的 netpoll，著名的 gnet，还有 java 的 netty 都是多 reactor 多线程模式。

字节的 netpoll 主要是自己实现用了一个协程池处理 I/O 事件，以及在内存上处理更加高性能（零拷贝技术），他的 readme 画了个大饼说要支持 io_uring 不知道得等到猴年马月了。

另外说一下，gnet 的多路复用监听使用了 ET 模式，这也就使它具备了更高的性能，源码我还没看过，更多的我不知道了🤣。

而 Proactor 模式将所有的 I/O 操作交给主线程和内核来处理，工作线程仅负责业务逻辑，什么意思？意思是工作线程只需要接受已经处理好的数据，然后负责逻辑处理就行了，而不需要 I/O 操作。
Proactor 其实是真正的异步，没有 reactor 使用广泛个人觉得一个原因是早期的 linux 对于异步 I/O 支持还不完善，还没有 io_uring，而 epoll 的生态更加完善成熟，所以就采取了 reactor 模式。
