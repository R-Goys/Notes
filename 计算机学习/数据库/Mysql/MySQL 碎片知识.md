...
最后，我们进入到了叶子节点，也就是挂着数据的节点，每个行数据都是和对应的索引绑定在一起，也就是说，结构是一样的，还是通过链表绑在一起，我们可以先将叶子节点加载到内存中，然后通过页目录进行二分查找，最终找到对应的行。而在分组内，肯定只能实现遍历查找了。
...
3. 隔离性：事务应该可以防止多个事务并发执行时导致的数据不一致，不会相互干扰。（MVCC 和锁机制实现）
4. 持久性：事务执行完成之后，数据持久保存。（redo log 实现）
对于其他三者保证了一致性这句话，我的理解是一致性意味着事务操作前后不能出现数据不一致的情况，比如转账这个操作最终对应的结果不能多转也不能少转，扣款也是；除此之外我们事务做出的操作不能在崩溃之后一部分执行成功，一部分执行失败，必须要进行回滚等操作来保证操作不会混乱，其实这个原子性也是实现了一部分的一致性的，他要求我在一个事务里面扣减某个用户的余额，而另一个用户上增加余额，不会出现只扣减或者只增加了余额的情况；而我们的事务还需要将其持久化到磁盘里面，比如我们事务执行过程中，突然宕机或者断电了，然后崩溃恢复的时候，提交过的结果没有保存就会全部丢失或者部分丢失，这是不符合一致性的。

所以综上，我们一致性需要面对的问题：一个事务里面的操作要么全部执行，要么全部失败，否则会发生不一致的情况；在并发场景下，如果不做隔离，就可能出现常见的并发问题，比如多个线程同时读取到然后将他 ++ 并赋值，此时就出现了覆盖的情况，这也是不被允许的，所以就需要隔离性；而持久性则是帮助我们进行事务提交后的结果保存，以及崩溃恢复做准备。
...
2. 读已提交（read committed）：一个事务只能看见另一个事务已经提交的数据，相当于创建了一个 read view ，每次读取数据都会创建一个 read view 快照。这里仅仅解决了脏读的问题。
...
binlog 有三种文件格式 `STATEMENT`，`ROW`，`MIXED` ，`statement` 是默认文件格式，会记录 SQL 语句，而 `ROW` 会记录行数据的修改记录，我们的 `ROW` 在频繁的更新中会记录多个修改记录，而 `statement` 只会记录 SQL 语句，所以第一种的占用更小，而第三种则是混合模式，根据不同的情况来做选择。
...
我们的变长字段长度列表和 NULL 值列表都是逆序存储，我们的记录头信息中指向下一行记录的指针位于记录头信息和真实数据的中间，相对来讲，读取排在前面的列的值以及排在前面的变长字段长度或者 NULL 值字段的距离会比较短，可能位于一个 cpu cache line 中，可以优化读取性能。
...
举个例子，Insert 语句如果在有间隙锁之间的间隙插入数据，此时他的隐式锁会升级成一个插入意向锁，他与间隙锁是冲突的；另外一种情况，当我们插入一个与现有数据主键索引冲突的数据时，也会升级成一个记录锁，从而其他事务无法在此时修改或删除这一个数据。那么二级唯一索引呢？此时会升级为一个间隙锁；当我们在两个事务中插入一条唯一二级索引冲突的数据，后插入的事务会等待，此时先插入的事务会在这个插入的数据加上记录锁，防止其他事务更改，而第二个事务则希望加上一个临键锁，此时就会产生冲突，导致后插入的事务等待。