...
当我们 `memtable` 转换成 `sstable` 时，总是会将这个 sstable 追加到第一个层级 L1，这也意味着，L1 是无序的，在之后的 L2、L3 则都是有序的，当我们的 L1 中的 `sstable` 到达一定的大小，就会开始合并他会从 L1 中选取一个 `sstable`，将其合并到 L2 中，他会遍历 `sstable` 中的数据，将每个 key 分发到 L2 中不同数据范围的 `sstable` 中，这样就实现了有序。当我们读取数据时，先从 `memtable` 中读取，然后依次从 L1、L2... 按照层级顺序读取，由于 L1 是乱序的，所以如果 L1 非常大，那么我们的读性能也会相对较差。
