...
这样带来的好处就是写入性能非常好，能够充分利用顺序写的优势，那么读取效率呢？bitcask 的存储结构并没有使用 lsm 的分层结构，也不是通过内部自动 compact，而是采取了另一种方式——**全内存索引**，他将所有的键的数据存储的文件的最新索引放在内存中，当我们需要查询某个键值的时候，通过这个 keydir 找到这个键的值在磁盘上的最新位置，从而可以直接从磁盘上快速找到这个 key 的最新位置，进而读取到最新的数据。
