...
## 18. Goroutine是用户态的切换，但是如何切换指令指针的？

很简单，该书中，已经提过了这一段代码，我们是通过jmp指令间接的去修改我们的指令指针的，所以可以确确实实地在用户态就可以实现切换操作，而不需要陷入内核，尽管操作系统中的内核中也是jmp实现的跳转，这个没啥好说的，但是学过os的理解确实不一样了一点，就像提一下。
