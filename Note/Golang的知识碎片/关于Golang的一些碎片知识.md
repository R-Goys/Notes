...
bmap 定义只包含 key 的哈希值前八位的数字，以加速查询，为什么不直接比较 key 呢？因为一个 key 可能是一个大数字，也可能是很长的字符串，cpu 直接比较耗时相对于只比较一个字节，损耗明显低了不小；除此之外，考虑到局部性原则，key 哈希值前八位这一串是通过数组遍历进行比较的，cache 命中率高，所以性能也会更高，而直接比较 key 性能就会低很多。所谓拉链法，其实就是将 bmap bucket 当作一个链表。
