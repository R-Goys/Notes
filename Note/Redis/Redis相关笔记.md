...
2. 回到main函数，由 `initListeners()` 设置监听套接字并注册到事件循环，同时，在这里会为服务端事件监听注册回调函数，这个回调函数会将监听到的客户端也注册到事件之中，同时该客户端的回调函数和服务端的回调函数并不相同，客户端的回调函数是将返回的信息通过http相应返回给客户端，
3. 随后调用`aeMain()`来运行事件循环，开始监听客户端的事件。
