...
- 字段为 2 时，相对 0 更安全，每次都会将 log buffer 写入 logfile（这是操作系统的 Page Cache），但是刷盘依旧是每秒一次，此时只有操作系统崩溃或者机器断电才会导致数据丢失。
...
在我们仅仅使用 redo log 和 binlog 的时候，有可能会导致主从数据不一致，比如 binlog 已经持久化，但是 redo log 还没有持久化的时候宕机了，主节点的数据就延后了这种情况。
