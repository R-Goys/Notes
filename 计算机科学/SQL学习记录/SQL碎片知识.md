...
此时会将 phone 转换为 int，而不是将 2 转换为 varchar！由于 varchar 比较方法和数值不一样，如果此时走索引的代价无异于新建一个索引，所以此时会触发索引失效，而走全表扫描。与此同时，函数也是一个原理，当我们对 phone 套一层函数时，由于函数输出并不确定，使得索引也失效了，这一点和类型转换原理上差不多。当然，mysql 也存在优化器会对有些语句进行改写，最后还是可以走索引的。
...
前面提过早期仅凭 binlog 无法做到 crash-safe，于是有了 redolog 来帮助做到这点，但是这也意味着我们需要维护两个日志副本，而 redolog 是保证 crash-safe,而 binlog 是没有的，这也就间接导致了这两个日志可能会不一致！于是就有了————二阶段提交。
